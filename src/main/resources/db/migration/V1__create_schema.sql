...
DO $$
BEGIN
    IF NOT EXISTS (SELECT FROM pg_catalog.pg_roles WHERE rolname = 'permit_app') THEN
CREATE ROLE permit_app WITH LOGIN PASSWORD 'permit_app_2026';
END IF;
END
$$;
...
GRANT SELECT, INSERT, UPDATE, DELETE ON users TO permit_app;
GRANT SELECT, INSERT, UPDATE, DELETE ON permits TO permit_app;
GRANT SELECT, INSERT, UPDATE, DELETE ON permit_documents TO permit_app;
